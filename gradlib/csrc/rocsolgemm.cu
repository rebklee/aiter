...
// #ifdef __gfx908__
// // Uncomment ifdef and endif only if you need to undef the HIP_HALF ops below just for gfx908 and
// not for others
...
// (PYTORCH_ROCBLAS_VERSION_DECIMAL >= 242) 	#if USE_GEMM_FLAGS_FP16_ALT_IMPL 	  #ifdef
// ROCM_BACKWARD_PASS_GUARD 		flag = at::BackwardPassGuard::is_backward_pass() ?
...
// std::map<std::tuple<int, int, int, int, int, int>, std::vector<hipblasLtMatmulHeuristicResult_t>>
// heuristic_map;
...
std::endl;
    ...
    if (returnedAlgoCount == 1) {
      ...
      for (int sol { 0 }; sol < returnedAlgoCount; ++sol) {
        ...
        if (cout_print) {
          std::cout << "    Sol " << sol << ": average time per iter " << std::to_string(eventMs) <<
" ms";
        }
        if (bestMs > eventMs) {
          bestMs = eventMs;
...
#define GEMM_EX_ARGS                                                                              \
    r_handle, transpose_mat1 ? rocblas_operation_transpose : rocblas_operation_none,              \
        transpose_mat2 ? rocblas_operation_transpose : rocblas_operation_none, m, n, k, &one,     \
        ptrA, abcRtype, mat1_ld, ptrB, abcRtype, mat2_ld, &zero, ptrC, abcRtype, result_ld, ptrC, \
        abcRtype, result_ld, rocblas_datatype_f32_r, rocblas_gemm_algo_solution_index
                   ...
                   const int32_t solution_index)
{
    ...
    //           << " | (lda, ldb, ldc): " << mat1_ld << ", " << mat2_ld << ", " << result_ld <<
    //           std::endl;
